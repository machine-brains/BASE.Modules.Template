...
using Tests.Modules.KWMODULENAME.Static.Quality.Helpers;

namespace Tests.Modules.KWMODULENAME.Static.Quality.Reliability
{
	/// <summary>
	...
	public class ServiceConventionTests
	{
		...
		/// </summary>
		[Fact]
		[Trait(TestTraits.Quality, TestTraits.Iso25010.FunctionalSuitability.Completeness)]
			...
			foreach (var controller in controllerTypes)
			{
				foreach (var ctor in controller.GetConstructors(BindingFlags.Public | BindingFlags.Instance))
				{
					foreach (var param in ctor.GetParameters())
					{
						...
						{
							violations.Add(
								$"{controller.Name} → {param.ParameterType.Name} (no IHasLifecycle)");
						}
					}
				}
			}
		...
		/// </summary>
		[Fact]
		[Trait(TestTraits.Quality, TestTraits.Iso25010.Maintainability.Modularity)]
